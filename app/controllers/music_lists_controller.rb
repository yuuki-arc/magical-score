  ...
  private
    ...
    def submit_login
      ...
      @@agent.page.form_with(name: 'loginActionForm') do |form|
        form.field_with(name: 'accessCode').value = @current_user.access_code #Setting.login_id
        form.field_with(name: 'password').value = @current_user.password #Setting.login_password
        form.click_button
      end
